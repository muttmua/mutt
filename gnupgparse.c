...
static int parse_timestamp(char *p, time_t *timestamp)
{
  if (strchr(p, '-'))   /* gpg pre-2.0.10 used format (yyyy-mm-dd) */
  {
    ...
    if (mutt_atoi(y, &time.tm_year, 0) < 0)
      return -1;
    time.tm_year -= 1900;
    if (mutt_atoi(m, &time.tm_mon, 0) < 0)
      return -1;
    time.tm_mon -= 1;
    if (mutt_atoi(d, &time.tm_mday, 0) < 0)
      return -1;
  ...
  for (p = buf; p; p = pend)
  {
    ...
    switch (field)
    {
      ...
      case 12:                  /* key capabilities */
        muttdbg(2, "capabilities info: %s", p);
