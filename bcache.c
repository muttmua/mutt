...
static int bcache_path(ACCOUNT *account, const char *mailbox, body_cache_t *bcache)
{
  ...
  {
    muttdbg(1, "URL to string failed");
    return -1;
  }
...
FILE* mutt_bcache_get(body_cache_t *bcache, const char *id)
{
  ...
  muttdbg(3, "bcache: get: '%s': %s", mutt_b2s (path),
              fp == NULL ? "no" : "yes");
...
out:
  muttdbg(3, "bcache: put: '%s'", mutt_b2s (path));
  mutt_buffer_pool_release (&path);
  return fp;
}
...
int mutt_bcache_move(body_cache_t* bcache, const char* id, const char* newid)
{
  ...
  muttdbg(3, "bcache: mv: '%s' '%s'", mutt_b2s (path), mutt_b2s (newpath));
...
int mutt_bcache_del(body_cache_t *bcache, const char *id)
{
  ...
  muttdbg(3, "bcache: del: '%s'", mutt_b2s (path));
                                    ...
                                    void *data), void *data)
{
  ...
  muttdbg(3, "bcache: list: dir: '%s'", bcache->path);
...
out:
  ...
  muttdbg(3, "bcache: list: did %d entries", rc);
  return rc;
}
