               ...
               const char *prefix)
{
  ...
  if (flags & CH_REORDER)
  {
    ...
    {
      muttdbg(3, "Reorder list: %s", t->data);
      hdr_count++;
    }
  }

  muttdbg(1, "WEED is %s", (flags & CH_WEED) ? "Set" : "Not Set");
  ...
  while (ftello (in) < off_end)
  {
      ...
      if (flags & CH_REORDER)
      {
        ...
        for (t = HeaderOrderList, x = 0 ; (t) ; t = t->next, x++)
        {
          ...
          if (!ascii_strncasecmp (buf, t->data, hdr_order_len))
          {
            ...
            muttdbg(2, "Reorder: %s matches %s", t->data, buf);
          }
        }
        if (match != -1)
          x = match;
    ...
    if (!ignore)
    {
      muttdbg(2, "Reorder: x = %d; hdr_count = %d", x, hdr_count);
      if (!this_one)
      {
                   ...
                   int chflags)
{
  ...
  {
    muttdbg(1, "_mutt_copy_message failed to detect EOF!");
    r = -1;
  }
