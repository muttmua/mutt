...
int imap_browse (const char* path, struct browser_state* state)
{
  ...
  {
    int rc;
    muttdbg(3, "imap_browse: mbox: %s", mbox);
    ...
    if (n > 0)                  /* "aaaa/bbbb/" -> "aaaa" */
    {
      ...
      if (showparents)
      {
        muttdbg(3, "adding parent %s", mbox);
...
int imap_mailbox_create (const char* folder, BUFFER *result)
{
  ...
  {
    muttdbg (1, "imap_mailbox_create: Bad starting path %s", folder);
    return -1;
  }
...
int imap_mailbox_rename(const char* mailbox, BUFFER *result)
{
  ...
  {
    muttdbg (1, "imap_mailbox_rename: Bad source mailbox %s", mailbox);
    return -1;
  }
                                   ...
                                   struct browser_state* state, short isparent)
{
  ...
  {
    muttdbg (2, "current folder %s makes no sense", state->folder);
    return -1;
  }
